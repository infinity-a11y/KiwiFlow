...
process.env.NODE_CONFIG_DIR = path.join(app.getAppPath(), 'config');
...
console.log("Config R.path.portable:", config.get("R.path.portable"));
console.log("Config R.app:", config.get("R.app"));
...
var port = parseInt(config.get("R.port"))
	if(isNaN(port)){
		error(`R.port is not a Number change config settings got ${config.get("R.port")}, using port 3838`, "Port Warning")
		port = 3838
	}
const execPortable = !!config.get("R.isPortable")
...
function startR() {
  const appPath = path.normalize(path.join(app.getAppPath(), config.get("R.app")));
  const execPath = path.normalize(path.join(app.getAppPath(), config.get("R.path.portable").replace("./", "")));
  const port = parseInt(config.get("R.port")) || 3838;

  console.log("execPath:", execPath);
  console.log("appPath:", appPath);

  const escapedAppPath = appPath.replace(/\\/g, "\\\\");
  const command = `options(shiny.port=${port});shiny::runApp('${escapedAppPath}')`;
  console.log("R command:", command);

  childProcess = child.spawn(execPath, [
    "-e",
    command,
  ], {
    cwd: app.getAppPath(),
  ...
  childProcess.stderr.on('data', data => {
    console.warn(`Rerr: ${data}`);
    // Try to parse R errors for more context
    if (data.toString().includes("Error")) {
      console.error("Shiny startup failed:", data.toString());
    }
  });
  childProcess.on('error', err => console.error("Spawn error:", err));
